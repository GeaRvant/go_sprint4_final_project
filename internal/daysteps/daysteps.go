...
func parsePackage(data string) (int, time.Duration, error) {
	...
	if elementToInt <= 0 {
		return 0, 0, fmt.Errorf("Ошибка: некорректное количество шагов (%d)", elementToInt)
	}
...
func DayActionInfo(data string, weight, height float64) string {
	...
	if walkSteps <= 0 {
		return fmt.Sprintf("Ошибка: некорректное количество шагов (%d)", walkSteps)
	}
	if walkDuration <= 0 {
		return fmt.Sprintf("Ошибка: длительность должна быть больше нуля (%d)", walkDuration)
	}
